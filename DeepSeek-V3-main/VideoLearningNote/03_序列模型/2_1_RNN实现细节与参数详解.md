...
```
传统前馈神经网络（Feedforward Neural Network）:

输入层 x
  ↓ (通过 W[1])
隐藏层1: a[1] = g(W[1]·x + b[1])
  ↓ (通过 W[2])
隐藏层2: a[2] = g(W[2]·a[1] + b[2])
  ↓ (通过 W[3])
输出层: y = g(W[3]·a[2] + b[3])
...
其中:
- W[l]: 第 l 层的权重矩阵, 维度 [n[l], n[l-1]]
- a[l-1]: 前一层的激活值（输出）
- b[l]: 第 l 层的偏置向量
- g: 激活函数（ReLU, sigmoid, tanh 等）
...
x^<1>      x^<2>      x^<3>      x^<4>    (输入序列)
 ↓          ↓          ↓          ↓
[RNN]──→  [RNN]──→  [RNN]──→  [RNN]     (隐藏层，水平循环)
h^<0> h^<1>    h^<2>    h^<3>    h^<4>   (隐藏状态，携带历史)
 ↓          ↓          ↓          ↓
y^<1>      y^<2>      y^<3>      y^<4>    (输出序列)

关键特点:
✅ 数据既向前流动（时间步），也向下流动（层间）
✅ 每个时间步依赖于：当前输入 x^<t> + 前一时刻的隐藏状态 h^<t-1>
✅ 有循环连接（水平箭头 →）
✅ 有"记忆"机制（h^<t> 包含历史信息）
```
...
其中:
- W_hh: 隐藏到隐藏权重, 维度 [n_h, n_h]  ← 新增！历史信息传递
- W_xh: 输入到隐藏权重, 维度 [n_h, n_x]  ← 类似传统网络的 W
- h^<t-1>: 前一时刻的隐藏状态（历史记忆）  ← 新增！
- x^<t>: 当前时刻的输入
- b_h: 隐藏层偏置
...
RNN:
输入来源: 双重（前一时刻的隐藏状态 + 当前时刻的输入）
公式: h^<t> = tanh(W_hh·h^<t-1> + W_xh·x^<t> + b_h)
                   └────┬────┘    └────┬────┘
                   历史信息        当前输入

类比:
传统网络 = 接力赛（只接棒，向下传）
RNN = 一边接棒（h^<t-1>），一边看路标（x^<t>）
```
...
RNN:
每个时间步两个权重矩阵:
1. W_hh ∈ ℝ^(n_h × n_h)  ← 处理历史信息 h^<t-1>
2. W_xh ∈ ℝ^(n_h × n_x)  ← 处理当前输入 x^<t>
...
参数量对比:
假设 n_h = 512, n_x = 10000
传统网络 (单层): W[1] = 512 × 前一层维度
RNN (单时间步): W_hh + W_xh = 512×512 + 512×10000
                                 ≈ 0.26M + 5.12M = 5.38M 参数
```
...
RNN:
参数在"时间步之间"共享
所有时间步使用相同的 W_hh, W_xh, b_h

示例:
时间步 t=1: 使用 W_hh, W_xh
时间步 t=2: 使用 W_hh, W_xh (相同参数!)
时间步 t=3: 使用 W_hh, W_xh (相同参数!)
...
...
特点:
- 水平: 循环连接（recurrent），h^<t-1> → h^<t>
- 垂直: 前馈连接（feedforward），x^<t> → h^<t> → y^<t>
- 有反馈（h 会回到自己）
```
...
示例:
隐藏层: ReLU (避免梯度消失)
输出层: softmax (多分类), sigmoid (二分类)

RNN:
激活函数: 引入非线性 + 控制信息流
常用: tanh（几乎是标准）
原因:
1. tanh 输出范围 [-1, 1]，零中心化，有利于梯度传播
2. tanh 可以表示"正负"信息（sigmoid 只能 [0,1]）
3. 历史原因（后续 LSTM/GRU 会改进）
...
────────────────────────────────────→ 时间（关键维度！）
 ...
 x^<1>    x^<2>    x^<3>    x^<4>  (输入序列)
     ↓        ↓        ↓        ↓
 [RNN]──→[RNN]──→[RNN]──→[RNN]      (隐藏层，循环连接)
 h^<1>    h^<2>    h^<3>    h^<4>   (隐藏状态)
     ↓        ↓        ↓        ↓
 y^<1>    y^<2>    y^<3>    y^<4>   (输出序列)
...
def feedforward_layer(x, W, b):
    """
    传统前馈层的前向传播

    参数:
        x: 输入, shape [n_input, 1]
        W: 权重矩阵, shape [n_output, n_input]
        b: 偏置, shape [n_output, 1]

    返回:
        a: 激活值, shape [n_output, 1]
    """
    # 线性变换
    z = np.dot(W, x) + b  # [n_output, n_input] @ [n_input, 1] = [n_output, 1]
...
# 示例: 3层前馈网络
n_input, n_h1, n_h2, n_output = 10, 5, 3, 2
...
W3 = np.random.randn(n_output, n_h2) * 0.01
b3 = np.zeros((n_output, 1))

# 前向传播（单个样本）
x = np.random.randn(n_input, 1)

a1 = feedforward_layer(x, W1, b1)   # 输入 → 隐藏层1
a2 = feedforward_layer(a1, W2, b2)  # 隐藏层1 → 隐藏层2
y = feedforward_layer(a2, W3, b3)   # 隐藏层2 → 输出

print(f"输入 x shape: {x.shape}")      # (10, 1)
print(f"隐藏层1 a1 shape: {a1.shape}")  # (5, 1)
print(f"隐藏层2 a2 shape: {a2.shape}")  # (3, 1)
print(f"输出 y shape: {y.shape}")       # (2, 1)
...
def rnn_cell(x, h_prev, W_hh, W_xh, b_h):
    """
    RNN 单元的前向传播

    参数:
        x: 当前输入, shape [n_x, 1]
        h_prev: 前一时刻隐藏状态, shape [n_h, 1]  ← 新增！
        W_hh: 隐藏到隐藏权重, shape [n_h, n_h]    ← 新增！
        W_xh: 输入到隐藏权重, shape [n_h, n_x]
        b_h: 偏置, shape [n_h, 1]

    返回:
        h_next: 当前隐藏状态, shape [n_h, 1]
    """
    # 双输入线性变换
    h_contribution = np.dot(W_hh, h_prev)  # 历史信息
    x_contribution = np.dot(W_xh, x)       # 当前输入

    z = h_contribution + x_contribution + b_h
...
# 初始化参数（所有时间步共享）
W_hh = np.random.randn(n_h, n_h) * 0.01  ← 新增！
...
# 前向传播（序列）
h = np.zeros((n_h, 1))  # 初始隐藏状态 h^<0>
H = []

for t in range(T):
    h = rnn_cell(X[t], h, W_hh, W_xh, b_h)  # 关键：h 作为输入和输出
    H.append(h)
    print(f"时间步 {t+1}: h shape = {h.shape}")

# 关键观察:
# ✅ 参数 W_hh, W_xh 在所有时间步共享
# ✅ 双向数据流: h^<t-1> → h^<t> (循环) 和 x^<t> → h^<t> (前馈)
# ✅ h 既是输入又是输出（循环连接）
# ✅ 两个权重矩阵: W_hh (历史) + W_xh (输入)
```
...
总计: 100,352 + 128 + 8,192 + 64 + 640 + 10 ≈ 109,386 个参数
...
├─ W_xh: 512 × 10000 = 5,120,000  ← 主要参数！
├─ W_hy: 2 × 512 = 1,024
├─ b_h: 512
└─ b_y: 2

总计: 262,144 + 5,120,000 + 1,024 + 512 + 2 ≈ 5,383,682 个参数

关键:
✅ 参数量与序列长度 T 无关（参数共享）
✅ 主要参数在 W_xh (约 95%)
✅ 可以处理任意长度序列（T = 10 或 T = 1000 都用相同参数）
```
...
| 维度 | 传统前馈网络 | RNN |
|------|-------------|-----|
| **输入来源** | 单一（前一层输出） | 双重（历史状态 + 当前输入） |
| **权重矩阵** | 每层一个 W[l] | 每时间步两个 W_hh, W_xh |
| **参数共享** | 层间不共享 | 时间步间共享 |
| **数据流向** | 单向（层间向下） | 双向（时间步水平 + 层间垂直） |
| **记忆机制** | ❌ 无记忆 | ✅ 有记忆（h^<t> 携带历史） |
| **激活函数** | 自由选择（ReLU 常用） | 通常 tanh |
| **适用任务** | 分类、回归（独立样本） | 序列建模（相互依赖） |
| **序列长度** | 不适用 | 可变长度（参数共享） |
| **参数量** | 由网络深度决定 | 与序列长度无关 |
...
实际上，拼接后的矩阵 W = [W_hh | W_xh] 在实现时是常用技巧，
但概念上仍然是两个独立的权重矩阵！
```

---

### ⚠️ 重要澄清：RNN 的输出与 Loss 计算

#### **问题：每个时间步都计算输出 y 和 Loss 吗？**
...
```
RNN 有多种输入输出模式（Sequence-to-Sequence 的变体）：

模式 1: Many-to-Many (每个时间步都有输出)
────────────────────────────────────────
t=1      t=2      t=3      t=4
 ↓        ↓        ↓        ↓
x^<1>    x^<2>    x^<3>    x^<4>    (输入序列)
...
[RNN]──→[RNN]──→[RNN]──→[RNN]      (隐藏层)
 ↓        ↓        ↓        ↓
y^<1>    y^<2>    y^<3>    y^<4>    (每个时间步都有输出)
 ↓        ↓        ↓        ↓
L^<1>    L^<2>    L^<3>    L^<4>    (每个时间步都计算 loss)

总 Loss: L = L^<1> + L^<2> + L^<3> + L^<4>
...
total_loss = 0
for t in range(T):
    h[t] = rnn_cell(x[t], h[t-1], W_hh, W_xh, b_h)
    y[t] = np.dot(W_hy, h[t]) + b_y  ← 每步都计算输出
    loss[t] = compute_loss(y[t], y_true[t])  ← 每步都计算 loss
    total_loss += loss[t]
```
...
# 只在最后一步计算
y = np.dot(W_hy, h[T-1]) + b_y  ← 只有最后一步
...
代码示例:
# 第一步有输入，后续步骤使用前一步的输出
h[0] = rnn_cell(x, h_init, W_hh, W_xh, b_h)
y[0] = np.dot(W_hy, h[0]) + b_y

for t in range(1, T):
    h[t] = rnn_cell(y[t-1], h[t-1], W_hh, W_xh, b_h)  ← 用输出作为输入
...
```
多层 RNN（Stacked/Deep RNN）:

层与层之间的传递是"垂直的"，每个时间步在所有层完成计算后才进行下一时间步
...
输入 x^<1>
    ↓ (输入到第1层)
第1层: h_1^<1> = tanh(W_hh_1·h_1^<0> + W_xh_1·x^<1> + b_h_1)
    ↓ (第1层的隐藏状态作为第2层的输入)
第2层: h_2^<1> = tanh(W_hh_2·h_2^<0> + W_xh_2·h_1^<1> + b_h_2)
    ↓ (第2层的隐藏状态作为第3层的输入)
第3层: h_3^<1> = tanh(W_hh_3·h_3^<0> + W_xh_3·h_2^<1> + b_h_3)
    ↓ (只有最后一层计算输出)
输出: y^<1> = W_hy·h_3^<1> + b_y  ← 只有最顶层计算输出！

然后才进行时间步 t=2...

关键要点:
✅ 每层都有自己的隐藏状态 h_l^<t>
✅ 下层的隐藏状态 h_l^<t> 作为上层的"输入"（类似 x^<t>）
✅ 只有最顶层计算输出 y^<t>
✅ Loss 只在最顶层的输出计算
```
...
──────────────────────────────────────────────────────
层3:  h_3^<1> ────→   h_3^<2> ────→   h_3^<3>
        ↓ (输出)        ↓ (输出)        ↓ (输出)
       y^<1>           y^<2>           y^<3>
        ↑ (来自层3)     ↑               ↑
──────────────────────────────────────────────────────
层2:  h_2^<1> ────→   h_2^<2> ────→   h_2^<3>
        ↑ (来自层2)     ↑               ↑
──────────────────────────────────────────────────────
层1:  h_1^<1> ────→   h_1^<2> ────→   h_1^<3>
        ↑ (来自输入)    ↑               ↑
──────────────────────────────────────────────────────
输入:  x^<1>           x^<2>           x^<3>
...
#### **代码实现：多层 RNN 的输出和 Loss**

```python
import numpy as np

def stacked_rnn_forward(X, params, num_layers):
    """
    多层 RNN 的前向传播
    ...
    """
    T = len(X)
    n_h = params[0]['W_hh'].shape[0]

    # 初始化所有层的隐藏状态
    H = [np.zeros((n_h, 1)) for _ in range(num_layers)]
    all_H = [[] for _ in range(num_layers)]
    Y = []
    ...
    for t in range(T):
        ...
        for l in range(num_layers):
            # 当前层的计算
            H[l] = np.tanh(
                np.dot(params[l]['W_hh'], H[l]) +  # 本层的循环连接
                np.dot(params[l]['W_xh'], layer_input) +  # 接收下层输入
                params[l]['b_h']
            )
            all_H[l].append(H[l].copy())

            # 本层的输出作为下一层的输入
            layer_input = H[l]

        # 只有最顶层计算输出 ← 关键！
        y_t = np.dot(params[-1]['W_hy'], H[-1]) + params[-1]['b_y']
        Y.append(y_t)
...
# 计算 Loss（根据任务类型）
def compute_loss_many_to_many(Y, Y_true):
    """Many-to-Many: 每个时间步都计算 loss"""
    total_loss = 0
    for t in range(len(Y)):
        loss_t = cross_entropy(Y[t], Y_true[t])  # 每个时间步的 loss
        total_loss += loss_t
    return total_loss / len(Y)  # 平均 loss


def compute_loss_many_to_one(Y, Y_true):
    """Many-to-One: 只有最后一个时间步计算 loss"""
    return cross_entropy(Y[-1], Y_true)  # 只用最后一步 ← 关键！
...
Y, all_H = stacked_rnn_forward(X, params, num_layers)

# 根据任务选择 loss 计算方式
if task_type == 'many_to_many':
    loss = compute_loss_many_to_many(Y, Y_true)
elif task_type == 'many_to_one':
    loss = compute_loss_many_to_one(Y, Y_true)

print(f"层1 时间步1 隐藏状态: {all_H[0][0].shape}")  # (n_h, 1)
print(f"层2 时间步1 隐藏状态: {all_H[1][0].shape}")  # (n_h, 1)
print(f"层3 时间步1 隐藏状态: {all_H[2][0].shape}")  # (n_h, 1)
print(f"时间步1 输出: {Y[0].shape}")  # (n_y, 1)
print(f"总 Loss: {loss}")
```
...
关键:
✅ 梯度从每个时间步的 loss 反向传播
✅ 梯度沿着时间维度反向流动（从 t=T 到 t=1）
✅ 所有时间步的梯度累加到同一个参数（因为参数共享）
✅ 最终更新: W ← W - α·(∂L^<1>/∂W + ∂L^<2>/∂W + ∂L^<3>/∂W)
...
for t in reversed(range(T)):
    # 计算当前时间步的梯度
    dh[t] = dy[t] @ W_hy.T  # 从输出梯度
    if t < T-1:
        dh[t] += dh_next @ W_hh.T  # 加上下一时刻传回的梯度

    # 累加参数梯度（所有时间步共享参数）
    dW_hh += dh[t] @ h[t-1].T  ← 累加！
    dW_xh += dh[t] @ x[t].T    ← 累加！

    dh_next = dh[t]  # 传递给前一时刻
...
| 问题 | 答案 |
|------|------|
| **每个时间步都计算输出 y 吗？** | 取决于任务：Many-to-Many 每步都算，Many-to-One 只算最后一步 |
| **每个时间步都计算 Loss 吗？** | 取决于任务：Many-to-Many 每步都算，Many-to-One 只算最后一步 |
| **多层 RNN 哪层计算输出？** | 只有最顶层（最后一层）计算输出 y^<t> |
| **下层的隐藏状态传递给上层吗？** | ✅ 是的！h_l^<t> 作为第 l+1 层的输入（垂直传递） |
| **Loss 如何反向传播？** | 通过 BPTT，梯度沿时间反向流动，所有时间步的梯度累加 |
| **参数如何更新？** | 累加所有时间步的梯度后，一次性更新（因为参数共享） |
...
```
❌ 误解 1: "每个时间步都必须计算输出和 loss"
✅ 正确: 取决于任务类型。情感分析只需要最后一步的输出。

❌ 误解 2: "多层 RNN 每层都计算输出"
✅ 正确: 只有最顶层计算输出，中间层的隐藏状态传递给上层。

❌ 误解 3: "Loss 在每层分别计算"
✅ 正确: Loss 只在最顶层的输出计算，然后通过 BPTT 反向传播。

❌ 误解 4: "每个时间步的参数独立更新"
✅ 正确: 所有时间步共享参数，梯度累加后一次性更新。

❌ 误解 5: "h^<t> 传递给下一层"
✅ 正确:
   - 水平传递: h^<t> → h^<t+1> (同一层，下一时刻)
   - 垂直传递: h_l^<t> → 作为第 l+1 层的输入（同一时刻，上一层）
```
